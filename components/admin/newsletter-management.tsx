...
export function NewsletterManagement() {
  ...
  const fetchData = async () => {
    try {
      ...
      const { count: subCount, error: subError } = await supabase.from("newsletter_subscribers").select("*", { count: "exact", head: true })
  ...
  const handleSend = async (id: string) => {
    ...
    try {
      ...
      const supabase = createClient();
      const { error } = await supabase.from("newsletters").update({ status: "sent", sent_at: new Date().toISOString() }).eq("id", id);
